<x-layout>
    <main class="flex justify-center items-center my-20 z-20 relative">
        <div class="border bg-gray-50 p-10 rounded w-1/2 shadow-xl">
            ...
            <form method="POST" action="/post-job/{{ $post->id }}" enctype="multipart/form-data"
                class="flex flex-col gap-y-6 text-lg">
                @csrf
                @method('PUT')
                <div>
                    <label for="company" class="inline-block mb-2">Company Name</label>
                    <input type="text" class="border border-gray-200 rounded p-2 w-full" name="company"
                        value="{{ $post->company }}" placeholder="e.g. Nova Nordisk" />
                ...
                <div>
                    ...
                    <input type="text" class="border border-gray-200 rounded p-2 w-full" name="title"
                        placeholder="e.g. Senior Laravel Developer" value="{{ $post->title }}" />
                    @error('title')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" class="border border-gray-200 rounded p-2 w-full" name="location"
                        placeholder="e.g. Copenhagen, Denmark" value="{{ $post->location }}" />
                    @error('location')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" class="border border-gray-200 rounded p-2 w-full" name="email"
                        value="{{ $post->email }}" placeholder="e.g. brian@gmail.com" />
                    @error('email')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" class="border border-gray-200 rounded p-2 w-full" name="website"
                        value="{{ $post->website }}" placeholder="e.g. http://test.com" />
                    @error('website')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="file" class="border border-gray-200 rounded p-2 w-full mb-2" name="logo" />
